    ...
    <div class="app-card p-6">
        <form action="{{ route('obligations.update', $obligation) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="app-form-group">
                    ...
                    <select id="partner_id" name="partner_id" class="app-select" required>
                        ...
                        @foreach($partners as $partner)
                            <option
                                value="{{ $partner->id }}"
                                {{ (string) old('partner_id', request('partner_id', $obligation->partner_id)) === (string) $partner->id ? 'selected' : '' }}
                            >
                                {{ $partner->name }}
                            </option>
                        @endforeach
                    ...
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <a
                            href="{{ route('partner-services.create', [
                                ...
                                'partner_id' => request('partner_id', $obligation->partner_id),
                            ]) }}"
                            id="add-service-link"
                            class="app-link text-sm"
                        >
                    ...
                    <select id="partner_service_id" name="partner_service_id" class="app-select">
                        ...
                        @foreach($services as $service)
                            <option
                                ...
                                {{ (string) old('partner_service_id', request('partner_service_id', $obligation->partner_service_id)) === (string) $service->id ? 'selected' : '' }}
                            >
                                {{ $service->name }}
                            </option>
                        @endforeach
                    ...
                    <select id="partner_contact_id" name="partner_contact_id" class="app-select">
                        ...
                        @foreach($contacts as $contact)
                            <option
                                ...
                                {{ (string) old('partner_contact_id', request('partner_contact_id', $obligation->partner_contact_id)) === (string) $contact->id ? 'selected' : '' }}
                            >
                                {{ $contact->full_name }}
                            </option>
                        @endforeach
                ...
                <div class="app-form-group md:col-span-2">
                    ...
                    <textarea
                        ...
                        rows="4"
                        class="app-textarea"
                    >{{ old('description', $obligation->description) }}</textarea>
                </div>

                <div class="app-form-group">
                    <label class="app-label" for="status">Status *</label>
                    <select id="status" name="status" class="app-select" required>
                        <option value="open" {{ old('status', $obligation->status) === 'open' ? 'selected' : '' }}>Open</option>
                        <option value="in_progress" {{ old('status', $obligation->status) === 'in_progress' ? 'selected' : '' }}>U tijeku</option>
                        <option value="waiting" {{ old('status', $obligation->status) === 'waiting' ? 'selected' : '' }}>Na čekanju</option>
                        <option value="done" {{ old('status', $obligation->status) === 'done' ? 'selected' : '' }}>Završeno</option>
                    </select>
                </div>
                ...
                <div class="app-form-group">
                    ...
                    <select id="priority" name="priority" class="app-select" required>
                        <option value="low" {{ old('priority', $obligation->priority) === 'low' ? 'selected' : '' }}>Nizak</option>
                        <option value="normal" {{ old('priority', $obligation->priority) === 'normal' ? 'selected' : '' }}>Normalan</option>
                        <option value="high" {{ old('priority', $obligation->priority) === 'high' ? 'selected' : '' }}>Visok</option>
                        <option value="urgent" {{ old('priority', $obligation->priority) === 'urgent' ? 'selected' : '' }}>Hitno</option>
                    </select>
                </div>
                ...
                <div class="app-form-group flex items-end">
                    <label class="inline-flex items-center gap-2">
                        <input
                            ...
                            name="is_recurring"
                            value="1"
                            {{ old('is_recurring', $obligation->is_recurring) ? 'checked' : '' }}
                        >
            ...
            <div
                ...
                style="{{ old('is_recurring', $obligation->is_recurring) ? '' : 'display:none;' }}"
            >
                <div class="app-form-group">
                    ...
                    <select id="recurrence_type" name="recurrence_type" class="app-select">
                        <option value="">-- odaberi --</option>
                        <option value="daily" {{ old('recurrence_type', $obligation->recurrence_type) === 'daily' ? 'selected' : '' }}>Dnevno</option>
                        <option value="weekly" {{ old('recurrence_type', $obligation->recurrence_type) === 'weekly' ? 'selected' : '' }}>Tjedno</option>
                        <option value="monthly" {{ old('recurrence_type', $obligation->recurrence_type) === 'monthly' ? 'selected' : '' }}>Mjesečno</option>
                        <option value="yearly" {{ old('recurrence_type', $obligation->recurrence_type) === 'yearly' ? 'selected' : '' }}>Godišnje</option>
                    </select>
                </div>
                ...
                <div class="app-form-group">
                    ...
                    <select id="remind_days_before" name="remind_days_before" class="app-select">
                        ...
                        <option value="0" {{ (string) old('remind_days_before', $obligation->remind_days_before) === '0' ? 'selected' : '' }}>Na dan roka</option>
                        <option value="1" {{ (string) old('remind_days_before', $obligation->remind_days_before) === '1' ? 'selected' : '' }}>1 dan prije</option>
                        <option value="3" {{ (string) old('remind_days_before', $obligation->remind_days_before) === '3' ? 'selected' : '' }}>3 dana prije</option>
                        <option value="7" {{ (string) old('remind_days_before', $obligation->remind_days_before) === '7' ? 'selected' : '' }}>7 dana prije</option>
                        <option value="14" {{ (string) old('remind_days_before', $obligation->remind_days_before) === '14' ? 'selected' : '' }}>14 dana prije</option>
                        <option value="30" {{ (string) old('remind_days_before', $obligation->remind_days_before) === '30' ? 'selected' : '' }}>30 dana prije</option>
                    </select>
                </div>
            </div>
